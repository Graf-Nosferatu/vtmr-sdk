...
public class DisciplineSummonSoul extends Discipline
{
	private static final String	DISCIPLINE_NAME	= "SummonSoul";

	// --------------------------------------------------------------------------------------------

	public DisciplineSummonSoul()
	{
	}
	...
	public int cast(int level, int casterGuid)
	{
		...
		try
		{
			int petGuid = casterThing.SpawnThingNear("wraith", 0.0f, 0);
			if(petGuid != 0)
			{
				...
				// start the effect
				petActor.AddActorEffectByLevel("ef_disc_summonsoul", SUMMONSOUL_DURATIONS[level], level, casterGuid, EI_FLAG_DISCIPLINE);
