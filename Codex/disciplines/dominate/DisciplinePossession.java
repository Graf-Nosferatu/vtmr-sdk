...
public class DisciplinePossession extends Discipline
{
	private static final String	DISCIPLINE_NAME	= "Possession";

	// --------------------------------------------------------------------------------------------

	public DisciplinePossession()
	{
	}
	...
	public int cast(int level, int casterGuid, int targetGuid)
	{
		...
		try
		{
				...
				)
			{
				...
				int effectGuid = targetThing.FindActorEffect("ef_disc_possession");
				if(effectGuid != 0)
					targetThing.RemoveActorEffect(effectGuid);
				
				// start the effect
				targetThing.AddActorEffectByLevel("ef_disc_possession", POSSESSION_DURATIONS[level], level, casterGuid, EI_FLAG_DISCIPLINE);
			}
			else
			{
