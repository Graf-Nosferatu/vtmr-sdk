...
public class DisciplineDarkHunter extends Discipline
{
	private static final String	DISCIPLINE_NAME	= "DarkHunter";

	// --------------------------------------------------------------------------------------------

	public DisciplineDarkHunter()
	{
	}
	...
	public int cast(int level, int casterGuid)
	{
		...
		try
		{
			int petGuid = casterThing.SpawnThingNear("darkhunter", 0.0f, 0);
			if(petGuid != 0)
			{
				...
				// start the effect
				petActor.AddActorEffectByLevel("ef_disc_darkhunter", DARKHUNTER_DURATIONS[level], level, casterGuid, EI_FLAG_DISCIPLINE);
