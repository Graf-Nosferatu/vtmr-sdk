/**
 * Discipline TheftOfVitae script.
...
public class DisciplineTheftOfVitae extends Discipline
{
	private static final String	DISCIPLINE_NAME	= "TheftOfVitae";

	// --------------------------------------------------------------------------------------------

	public DisciplineTheftOfVitae()
	{
	}
	...
	public boolean cancast(int level, int casterGuid, int targetGuid, float x, float y, float z, boolean bIsPos)
	{
		...
		if((targetThing.GetActorFlags() & THING_AF_NOBLEED) != 0)
		{
			...
			//CodexConsole.PrintNLS(casterGuid, 0, "RPG_DISC_NOTHEFTOFVITAE1", 0xFFFF00);
			return(false);
		}
		...
		if(targetThing.GetActorStat(ACTOR_STAT_BLOOD) < 1.0f)
		{
			...
			//CodexConsole.PrintNLS(casterGuid, 0, "RPG_DISC_NOTHEFTOFVITAE2", 0xFFFF00);
			return(false);
		}
	...
	public int cast(int level, int casterGuid, int targetGuid)
	{
		...
		{	
			// start the effect
			casterThing.AddActorEffectByLevel("ef_disc_theftofvitae1", 1000, level, casterGuid, EI_FLAG_DISCIPLINE);
			targetThing.AddActorEffectByLevel("ef_disc_theftofvitae2", 1000, level, casterGuid, EI_FLAG_DISCIPLINE);
